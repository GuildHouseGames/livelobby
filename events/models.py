...
class Event(models.Model):
    ...
    type = models.CharField(
        max_length=25, choices=TYPE_CHOICES, default="GAME"
        )

    def clean(self):
        # size validation
        if self.initial_size > self.max_size:
            raise ValidationError("The initial group size cannot be bigger "
                                  "than the max group size")
        if not self.max_size > self.initial_size:
            raise ValidationError("The max group size must be larger than "
        ...
        if (self.date == date.today()):
            if (self.time < now.time()):
                raise ValidationError("For a booking today, "
...
class Participant(models.Model):
    ...
    type = models.CharField(
        max_length=8, choices=PARTICIPANT_CHOICES, default="ATTENDEE"
        )
    ...
    def clean(self):
        ...
        if (joined_players.count()-1 + self.event.initial_size >=
                self.event.max_size):
